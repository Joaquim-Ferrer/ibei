...
public class BDInterface {
	...
	public String getAuctionDetails(String id) {
		String query = "SELECT id_leilao, username, cod_artigo, titulo, descricao, preco_maximo, deadline"
				+ " FROM leilao"
				+ " WHERE id_leilao = ?";
		...
		try(PreparedStatement stmt = connection.prepareStatement(query)) {
			...
			if(rs.first()) {
				...
				String preco_maximo = rs.getFloat("preco_maximo");
				String deadline = sr.getD
